...
description: >
  Use this skill when inbound leads arrive incomplete (missing company size, industry,
  revenue), the TAM list lacks data for scoring, or the CRM cannot route and segment
  without enrichment. Maps coverage gaps, compares single-source and waterfall provider
  strategies, and builds an integration roadmap with cost guardrails and quality gates.
  Produces a provider recommendation matrix, a waterfall architecture, and a go or no-go
  checklist. Rule: no single provider has full coverage; a waterfall across two or more
  providers in sequence beats any one source on match rate. Trigger phrases: data
  enrichment, leads come in with no company info, enrichment coverage, firmographic
